...
def train_one_epoch(config, train_loader, model, criterion, optimizer, epoch,
    ...
    aug = config.AUG
    print("funnction.py-aug.MIXUP_PROB:{}".format(aug.MIXUP_PROB))
    mixup_fn = Mixup(
    ...
    for i, (x, y) in enumerate(train_loader):
        ...
        if mixup_fn:
            # print("mixup_fn x:{} y:{}".format(x.shape,y.shape))
            x, y = mixup_fn(x, y)
...
def test(config, val_loader, model, criterion, output_dir, tb_log_dir,
    ...
    for i, (x, y) in enumerate(val_loader):
        ...
        top1.update(prec1, x.size(0))
        top5.update(prec5, x.size(0))
        predict.update(torch.max(outputs, dim = 1)[1].cpu().numpy())
        label.update(y.cpu().numpy())
        print(label.item.shape, predict.item.shape)
    ...
    if(predict):
        pre_micro = precision_score(label.item, predict.item, average='micro')
        pre_macro = precision_score(label.item, predict.item, average='macro')
        pre_weigh = precision_score(label.item, predict.item, average='weighted')

        re_micro = recall_score(label.item, predict.item, average='micro')
        re_macro = recall_score(label.item, predict.item, average='macro')
        re_weigh = recall_score(label.item, predict.item, average='weighted')

        f1_micro = f1_score(label.item, predict.item, average='micro')
        f1_macro = f1_score(label.item, predict.item, average='macro')
        f1_weigh = f1_score(label.item, predict.item, average='weighted')

        msg = 'precision_micro {pre_micro}\t' \
        'precision_macro {pre_macro}\t' \
        'precision_weigh {pre_weigh}\t' \
        'recall_micro {recall_micro}\t' \
        'recall_macro {recall_macro}\t' \
        'recall_weigh {recall_weigh}\t' \
        'f1_micro {f1_micro}\t' \
        'f1_macro {f1_macro}\t' \
        'f1_weigh {f1_weigh}\t'.format(
            pre_micro = pre_micro, pre_macro = pre_macro, pre_weigh = pre_weigh,
            recall_micro = re_micro, recall_macro = re_macro, recall_weigh = re_weigh,
            f1_micro = f1_micro, f1_macro = f1_macro, f1_weigh = f1_weigh
        )
        logging.info(msg)
        print(msg)
...
def test_ED(config, val_loader, model, criterion, output_dir, tb_log_dir,
         writer_dict=None, distributed=False, real_labels=None,
         valid_labels=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    ...
    logging.info('=> switch to eval ED mode')
    model.eval()
...
class TotalMeter:
    """append and stores the total and current label list"""
    def __init__(self):
        self.item = np.array([])

    def update(self, new_item):
        self.item = np.hstack((self.item, new_item))
