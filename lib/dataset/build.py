...
class SimpleDataset(Dataset):
    def __init__(self, root, transform = None):
        self.file_paths = [root + i for i in self.sort_path(os.listdir(root))]
    ...
    def sort_path(self, namelist):
        img_name = {}
        for i in namelist:
            name, number = i, int(i.split('.')[0])
            img_name[name] = number
        img_names = sorted(img_name.items(), key = lambda x:x[1])
        output = []
        for name, number in img_names:
            output.append(name)
        return output
# x = SimpleDataset('/root/cvt/DATASET/mthv2/ED/##悲哀愍垂許時蓮花德佛告蓮/')

def build_dataset(cfg, is_train):
    dataset = None
    if 'imagenet' in cfg.DATASET.DATASET:
        dataset = _build_imagenet_dataset(cfg, is_train)
    elif 'mthv2' in cfg.DATASET.DATASET:
        dataset = _build_mthv2_dataset(cfg, is_train)
    else:
...
def _build_mthv2_dataset(cfg, is_train):
    transforms = build_transforms(cfg, is_train)
    ...
    print('mth length dataset: {}'.format(len(dataset)))
    # save class_to_idx
    txt = open(cfg.OUTPUT_DIR + 'class_to_idx.txt', 'w')
    txt.write(str(dataset.class_to_idx))

    return dataset

def _build_ED_dataset(cfg, is_train, root):
    logging.info("=> ED dataset is building")
    transforms = build_transforms(cfg, is_train)

    dataset = SimpleDataset(root, transforms) 
...
def build_ED_dataloader(cfg, is_train=True, distributed=False, dataset=None):
    if is_train:
        batch_size_per_gpu = cfg.TRAIN.BATCH_SIZE_PER_GPU
        shuffle = True
    else:
